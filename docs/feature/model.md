...
### 6.4 주문 승인/생산 배정 도메인 규칙 (Phase 3/4 반영 예정)

주문 승인(`RESERVED` → `CONFIRMED`/`PRODUCING`) 이후 생산 배정과 재고 반영 흐름에 대해, 사용자가 확정한 아래 5가지 규칙을 예시 도메인(`Example/Model/`)이 지켜야 한다. 이 규칙들은 **아직 Controller/통합 계층(Phase 3/4)에서 구현되지 않았으며**, 향후 주문 승인 액션과 생산 배정 로직을 만들 때 반드시 반영해야 하는 계약으로 여기 문서에 먼저 못박아 둔다.

1. **생산수량은 수율을 고려해 산정한다.** 생산수량 = `ceil(부족분 / 수율)`. 예: 부족분 50, 수율 0.5 → 생산수량 100.
   - **이미 구현/검증 완료**: `Example/Model/ProductionQueueEntry.h`의 `CalculateActualProductionQuantity`가 이 계산을 그대로 구현하고 있다. Phase 3/4는 이 함수를 재사용하면 되며, 별도 계산 로직을 새로 만들 필요가 없다.

2. **실제 생산량은 수율을 다시 적용하지 않는다.** 1번에서 산정된 생산수량(예: 100EA)이 그대로 "실제 생산량"이다. 생산 완료 후 재고에 반영할 때 여기에 수율을 다시 곱하거나 나누는 이중 적용을 해서는 안 된다. 즉 생산 완료 시 재고 증가량은 정확히 `CalculateActualProductionQuantity`가 반환한 값과 같아야 한다.
   - 1번과 마찬가지로 계산 자체는 기존 구현이 이미 만족한다. 관건은 Phase 3/4에서 "생산 완료 → 재고 반영" 배선을 만들 때 이 값을 그대로 사용하고 별도로 수율을 재적용하지 않는 것이다.

3. **생산 라인은 시스템 전체에 1개뿐이다.** 여러 시료/주문에 대한 생산 요청이 큐에 쌓여도, 동시에 `PRODUCING` 상태일 수 있는 `ProductionQueueEntry`는 시스템 전체에서 최대 1개다. 나머지는 모두 `WAITING`(대기) 상태로 큐에서 순서를 기다린다. 생산 큐는 시료별로 여러 개 존재하는 것이 아니라 전역 FIFO 큐 하나이며, 향후 오케스트레이션 로직(Controller 또는 통합 계층)은 "현재 `PRODUCING`인 항목이 있으면 새 항목을 `PRODUCING`으로 전이시키지 않는다"는 불변식을 지켜야 한다.
   - 이 불변식은 아직 코드로 강제되어 있지 않다. Phase 3/4에서 주문 승인 → 생산 배정 로직을 구현할 때 이 제약을 검증하는 코드(또는 최소한 gtest 케이스)를 함께 추가해야 한다.

4. **재고 선점은 별도의 예약 수량 필드로 추적한다(재고 자체를 직접 차감하지 않는다).** 주문이 접수되어 재고 충당이 필요한 상황(재고가 있고 그 재고로 일부를 충당 가능한 경우)이 되면, `Sample`의 실제 재고(`QuantityGuard`)를 즉시 차감하는 것이 아니라, 별도의 "예약된 수량(reserved quantity)"을 추적하는 필드/구조로 이미 어떤 주문에 얼마가 배정되어 있는지를 기록한다. "가용 재고(주문에 배정 가능한 재고) = 실제 재고 − 예약된 수량"이라는 개념이 이 로직의 핵심이 된다.
   - 이 예약 수량 필드는 **아직 코드에 존재하지 않는다.** `Example/Model/Sample.h`(또는 관련 조합 로직)에 Phase 3/4에서 추가될 예정이다.
   - 구현 형태(`QuantityGuard`를 재사용해 "예약 수량"용으로 하나 더 두는 방식으로 할지, 아니면 별도의 정수 필드 + 자체 불변식 검증으로 할지)는 **아직 미정**이며, Phase 3/4 착수 시점에 구체적으로 설계한다. 지금 단계에서 코드를 미리 만들지 않는다.

5. **재고 판단은 항상 현재 실제 재고만 본다(미래 예측 없음).** 부족분/생산수량을 계산할 때, 현재 큐에 대기 중이거나 생산 중인 다른 항목들이 모두 완료되었을 때의 예상 재고를 미리 반영하지 않는다. 오직 의사결정 시점의 실제 재고(및 4번의 예약된 수량)만 사용한다.
   - 즉 "N번째 대기 항목이 끝나면 재고가 얼마가 될 것이다"와 같은 예측 계산은 하지 않는다. 이 규칙 역시 Phase 3/4에서 부족분 계산 로직을 구현할 때 지켜야 할 제약으로, 아직 코드에 반영되지 않았다.
