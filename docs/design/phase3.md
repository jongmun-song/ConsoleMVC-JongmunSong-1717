...
6. **PoC 예시 메뉴 트리 검증**: `Example/Controller/`에 예시 메뉴 트리(메인 메뉴 → 시료 관리/시료 주문/주문 승인·거절/모니터링/생산라인/출고 처리)를 `IController`/`MenuNode`/`NavigationLoop`로 구현하여, `Example/Model/`(Phase 1)과 `Example/View/`(Phase 2)를 실제로 연결한다(`../feature/controller.md` §7 참고). 코어(`Controller/`)는 수정하지 않는다.
   - **주문 승인 액션(`RESERVED` → `CONFIRMED`/`PRODUCING`) 구현 시 참고**: `../feature/model.md` §6.4에 확정된 5가지 도메인 규칙(생산수량 = `ceil(부족분/수율)`, 실제 생산량에 수율 재적용 금지, **생산 라인은 시스템 전체에 1개뿐**이므로 `PRODUCING` 상태 항목이 이미 있으면 새 항목을 `PRODUCING`으로 전이시키지 않을 것, 재고는 즉시 차감하지 않고 **예약 수량 필드**로 선점을 추적할 것, 부족분/생산수량 계산은 **현재 실제 재고만** 사용하고 큐의 다른 항목 완료를 미리 예측하지 않을 것)을 그대로 따른다. 예약 수량 필드의 구체 형태는 이 Phase에서 `Example/Model/Sample.h`(또는 관련 조합 로직)에 설계/추가한다.
